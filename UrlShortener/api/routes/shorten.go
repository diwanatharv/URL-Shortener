...
func ShortenURL(c *fiber.Ctx) error {
	...
	if !govalidator.IsURL(body.URL) {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "cannot parse url"})
	}
	...
	if val != "" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "URL custom short url is already used"})
	}
	if body.Expiry == 0 {
		body.Expiry = 24
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "cannot connect to the server"})
	}
	resp := response{
		URL:            body.URL,
		CustomShort:    "",
	...
	resp.XRateLimitRest = ttl / time.Nanosecond / time.Second
